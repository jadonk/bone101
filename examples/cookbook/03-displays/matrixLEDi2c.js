...
b.i2cWriteByte([port, matrix],  0x21); // Start oscillator (p10)		<5>
b.i2cWriteByte([port, matrix],  0x81); // Disp on, blink off (p11)
b.i2cWriteByte([port, matrix],  0xe7); // Full brightness (page 15)

b.i2cWriteBytes([port, matrix], 0x00, frown);			// <6>
sleep.usleep(time);

b.i2cWriteBytes([port, matrix], 0x00, neutral);
sleep.usleep(time);

b.i2cWriteBytes([port, matrix], 0x00, smile);
// Fade the display
var fade;
for(fade = 0xef; fade >= 0xe0; fade--) {		// <7>
    b.i2cWriteByte([port, matrix],  fade);
...
b.i2cWriteBytes([port, matrix], 0x04, [0xff]);
